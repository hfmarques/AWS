...
WORKDIR /src
COPY ["LambdaToConsumeFromSqs.csproj", "LambdaToConsumeFromSqs/"]
RUN dotnet restore "LambdaToConsumeFromSqs/LambdaToConsumeFromSqs.csproj"

WORKDIR "/src/LambdaToConsumeFromSqs"
COPY . .
RUN dotnet build "LambdaToConsumeFromSqs.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "LambdaToConsumeFromSqs.csproj" \
...
Execute unit tests
```
cd "LambdaToConsumeFromSqs/test/LambdaToConsumeFromSqs.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "LambdaToConsumeFromSqs/src/LambdaToConsumeFromSqs"
