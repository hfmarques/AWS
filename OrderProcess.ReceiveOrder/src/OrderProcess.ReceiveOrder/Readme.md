...
WORKDIR /src
COPY ["OrderProcess.ReceiveOrder.csproj", "OrderProcess.ReceiveOrder/"]
RUN dotnet restore "OrderProcess.ReceiveOrder/OrderProcess.ReceiveOrder.csproj"

WORKDIR "/src/OrderProcess.ReceiveOrder"
COPY . .
RUN dotnet build "OrderProcess.ReceiveOrder.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "OrderProcess.ReceiveOrder.csproj" \
...
Execute unit tests
```
cd "OrderProcess.ReceiveOrder/test/OrderProcess.ReceiveOrder.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "OrderProcess.ReceiveOrder/src/OrderProcess.ReceiveOrder"
