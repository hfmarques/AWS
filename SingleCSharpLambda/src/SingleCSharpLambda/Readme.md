...
WORKDIR /src
COPY ["SingleCSharpLambda.csproj", "SingleCSharpLambda/"]
RUN dotnet restore "SingleCSharpLambda/SingleCSharpLambda.csproj"

WORKDIR "/src/SingleCSharpLambda"
COPY . .
RUN dotnet build "SingleCSharpLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "SingleCSharpLambda.csproj" \
...
Execute unit tests
```
cd "SingleCSharpLambda/test/SingleCSharpLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "SingleCSharpLambda/src/SingleCSharpLambda"
